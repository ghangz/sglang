...
def test_top_k_top_p_sampling_from_probs_logits_top_k_first_alignment(
    ...
    samples = sgl_kernel_custom.sampling.top_k_top_p_sampling_from_logits(
        logits, k, p, filter_apply_order="top_k_first", generator=generator_logits
    )
    samples_ref = sgl_kernel_custom.sampling.top_k_top_p_sampling_from_probs(
...
def test_top_k_top_p_joint_sampling_from_probs(batch_size, vocab_size, p):
    ...
    for _ in range(num_trails):
        samples = sgl_kernel_custom.top_k_top_p_sampling_from_probs(
            normalized_prob,
            top_k_tensor,
            top_p_tensor,
            filter_apply_order="joint",
...
def test_top_p_renorm_probs(batch_size, vocab_size, p):
    ...
    renorm_prob = sgl_kernel_custom.top_p_renorm_prob(normalized_prob, p)
...
def test_top_k_renorm_probs(batch_size, vocab_size, k):
    ...
    renorm_prob = sgl_kernel_custom.top_k_renorm_prob(normalized_prob, k)
...
def test_min_p_sampling(batch_size, vocab_size, p):
    ...
    num_trails = 1000
    for _ in range(num_trails):
        samples = sgl_kernel_custom.min_p_sampling_from_probs(
            normalized_prob,
            min_p_tensor,
        )
