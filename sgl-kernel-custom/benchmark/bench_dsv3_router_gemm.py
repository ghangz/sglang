...
from sgl_kernel_custom import dsv3_router_gemm

# CI environment uses simplified parameters
if IS_CI:
    num_tokens_vals = [1]  # Only test 1 value in CI
    line_vals = ["sgl-kernel-custom-256"]  # Only test one implementation in CI
else:
    num_tokens_vals = [i + 1 for i in range(16)]  # Test 1-16 in full mode
    line_vals = ["torch-256", "sgl-kernel-custom-256", "torch-384", "sgl-kernel-custom-384"]
...
def benchmark_bf16_output(num_tokens, impl):
    ...
    if impl == "torch-256" or impl == "sgl-kernel-custom-256":
        N = 256
    elif impl == "torch-384" or impl == "sgl-kernel-custom-384":
        N = 384
    else:
    ...
    elif impl == "sgl-kernel-custom-256" or impl == "sgl-kernel-custom-384":
