...
import torch
from sgl_kernel_custom import FusedSetKVBufferArg, apply_rope_with_cos_sin_cache_inplace
...
class SglKernelRotaryEmbedding(RotaryEmbedding):
    def forward_cuda(
        ...
        assert (
            fused_set_kv_buffer_arg is None
        ), "fused_set_kv_buffer_arg is not supported for sgl-kernel-custom implementation"
        if self.cos_sin_cache.dtype != query.dtype:
            self.cos_sin_cache = self.cos_sin_cache.to(query.dtype)
        torch.ops.sgl_kernel_custom.rotary_embedding(
            positions,
            query,
            key,
            self.head_size,
