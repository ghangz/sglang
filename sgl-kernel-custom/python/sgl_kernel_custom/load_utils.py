...
def _load_architecture_specific_ops():
    ...
    logger.debug(
        f"[sgl_kernel_custom] GPU Detection: compute_capability = {compute_capability}"
    )

    # Get the directory where sgl_kernel_custom is installed
    sgl_kernel_dir = Path(__file__).parent
    logger.debug(f"[sgl_kernel_custom] sgl_kernel_custom directory: {sgl_kernel_dir}")
    ...
    logger.debug(f"[sgl_kernel_custom] Attempting to load {variant_name}")
    logger.debug(f"[sgl_kernel_custom] Looking for library matching pattern: {ops_pattern}")
    logger.debug(f"[sgl_kernel_custom] Found files: {raw_matching_files}")
    logger.debug(f"[sgl_kernel_custom] Prioritized files: {matching_files}")
    ...
    if matching_files:
        ...
        logger.debug(f"[sgl_kernel_custom] Found architecture-specific library: {ops_path}")
        try:
            ...
            logger.debug(f"[sgl_kernel_custom] Loading module from {ops_path}...")
            spec.loader.exec_module(common_ops)
            logger.debug(f"[sgl_kernel_custom] ✓ Successfully loaded {variant_name}")
            logger.debug(f"[sgl_kernel_custom] ✓ Module file: {common_ops.__file__}")
            return common_ops

        except Exception as e:
            previous_import_errors.append(e)
            logger.debug(
                f"[sgl_kernel_custom] ✗ Failed to load from {ops_path}: {type(e).__name__}: {e}"
            )
            # Continue to fallback
    else:
        logger.debug(
            f"[sgl_kernel_custom] ✗ Architecture-specific library not found matching pattern: {ops_pattern}"
        )
    ...
    logger.debug(f"[sgl_kernel_custom] Attempting fallback: looking for pattern {alt_pattern}")
    logger.debug(f"[sgl_kernel_custom] Found fallback files: {raw_alt_files}")
    logger.debug(f"[sgl_kernel_custom] Prioritized fallback files: {alt_matching_files}")

    if alt_matching_files:
        alt_path = Path(alt_matching_files[0])  # Use the first prioritized file
        logger.debug(f"[sgl_kernel_custom] Found fallback library: {alt_path}")
        try:
            ...
            logger.debug(f"[sgl_kernel_custom] Loading fallback module from {alt_path}...")
            spec.loader.exec_module(common_ops)
            logger.debug(f"[sgl_kernel_custom] ✓ Successfully loaded fallback library")
            logger.debug(f"[sgl_kernel_custom] ✓ Module file: {common_ops.__file__}")
            return common_ops
            ...
            previous_import_errors.append(e)
            logger.debug(
                f"[sgl_kernel_custom] ✗ Failed to load fallback from {alt_path}: {type(e).__name__}: {e}"
            )
    else:
        logger.debug(
            f"[sgl_kernel_custom] ✗ Fallback library not found matching pattern: {alt_pattern}"
        )

    # Final attempt: try standard Python import (for backward compatibility)
    logger.debug(
        f"[sgl_kernel_custom] Final attempt: trying standard Python import 'common_ops'"
    )
    try:
        import common_ops

        logger.debug(f"[sgl_kernel_custom] ✓ Successfully imported via standard Python import")
        logger.debug(f"[sgl_kernel_custom] ✓ Module file: {common_ops.__file__}")
        return common_ops
    except ImportError as e:
        previous_import_errors.append(e)
        logger.debug(f"[sgl_kernel_custom] ✗ Standard Python import failed: {e}")
    ...
    if cuda_version and cuda_version.startswith("13"):
        install_hint = (
            "pip install sgl-kernel-custom --index-url https://docs.sglang.ai/whl/cu130/"
        )
    else:
        install_hint = "pip install --upgrade sgl_kernel_custom"

    error_msg = f"""
[sgl_kernel_custom] CRITICAL: Could not load any common_ops library!
...
Please ensure sgl_kernel_custom is properly installed with:
{install_hint}
...
def _preload_cuda_library():
    ...
    for base in candidate_dirs:
        for lib_version in lib_versions:
            candidate = base / f"libruntime_cu.so"
            if candidate.exists():
                try:
    ...
    logger.debug("[sgl_kernel_custom] Could not preload CUDA runtime library")
