...
at::Tensor causal_conv1d_fwd_cpu(
    ...
    int64_t pad_slot_id,
    bool is_vnni) {
  RECORD_FUNCTION("sgl-kernel-custom::causal_conv1d_fwd_cpu", std::vector<c10::IValue>({x, weight, bias}));
...
at::Tensor causal_conv1d_update_cpu(
    ...
    int64_t pad_slot_id,
    bool is_vnni) {
  RECORD_FUNCTION("sgl-kernel-custom::causal_conv1d_update_cpu", std::vector<c10::IValue>({x, weight, bias}));
