...
try:
    from sgl_kernel_custom import flashmla_ops  # triggers TORCH extension registration
...
_IMPORT_ERROR = ImportError(
    "Failed to load sgl_kernel_custom.flashmla_ops extension. Ensure CUDA Driver >= 12.4"
)
...
def get_mla_metadata(
    ...
    if is_fp8_kvcache and topk is None:
        return torch.ops.sgl_kernel_custom.get_mla_decoding_metadata_dense_fp8.default(
            cache_seqlens,
            num_q_tokens_per_head_k,
            num_heads_k,
        )
    return torch.ops.sgl_kernel_custom.get_mla_decoding_metadata.default(
        cache_seqlens,
        num_q_tokens_per_head_k,
        num_heads_k,
        num_heads_q,
...
def flash_mla_with_kvcache(
    ...
    if indices is None and q.element_size() == 1:
        out, softmax_lse = torch.ops.sgl_kernel_custom.fwd_kvcache_mla_fp8.default(
            q,
            k_cache,
            head_dim_v,
            cache_seqlens,
        ...
        out, softmax_lse = torch.ops.sgl_kernel_custom.fwd_kvcache_mla.default(
            q,
            k_cache,
            head_dim_v,
            cache_seqlens,
...
def flash_mla_sparse_fwd(
    ...
    """
    results = torch.ops.sgl_kernel_custom.sparse_prefill_fwd.default(
