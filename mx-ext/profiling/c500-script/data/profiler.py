...
def find_fourth_triton(txt_in):
    # Open the file and read the lines
    with open(txt_in, 'r') as f:
        kernel_list = f.readlines()

    triton_count = 0  # Count of 'triton' occurrences
    fourth_triton_idx = -1  # The index of the fourth occurrence
    kernel_names = []  # List to store 'triton' kernel names
    kernel_times = []  # List to store 'triton' kernel times

    # Iterate through each line to check for 'triton'
    for line_idx, kernel in enumerate(kernel_list):
        name, time, v = kernel.strip().split(',')  # Extract kernel name and time
        kernel_names.append(name)
        kernel_times.append(time.split()[0])
        if name  == 'triton':
            triton_count += 1
            if triton_count == 4:
                fourth_triton_idx = line_idx  # Capture the index when 'triton' appears for the fourth time

    return fourth_triton_idx, kernel_names, kernel_times

# Example usage
txt_in = 'output8.trace.txt'  
...
if __name__ == '__main__':
    ...
    fourth_triton_idx, triton_names, triton_times = find_fourth_triton(txt_in)
    prefill_kernel_list  = kernel_list[:fourth_triton_idx]
    decoding_kernel_list = kernel_list[fourth_triton_idx:]
