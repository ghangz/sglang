...
topk_sigmoid_cpu(at::Tensor& hidden_states, at::Tensor& gating_output, int64_t topk, bool renormalize) {
  RECORD_FUNCTION("sgl-kernel-custom::topk_sigmoid_cpu", std::vector<c10::IValue>({hidden_states, gating_output}));
...
topk_softmax_cpu(at::Tensor& hidden_states, at::Tensor& gating_output, int64_t topk, bool renormalize) {
  RECORD_FUNCTION("sgl-kernel-custom::topk_softmax_cpu", std::vector<c10::IValue>({hidden_states, gating_output}));
  CHECK_INPUT(gating_output);
...
std::tuple<at::Tensor, at::Tensor> grouped_topk_cpu(
  ...
  RECORD_FUNCTION("sgl-kernel-custom::grouped_topk_cpu", std::vector<c10::IValue>({hidden_states, gating_output}));
  CHECK_INPUT(gating_output);
...
std::tuple<at::Tensor, at::Tensor> biased_grouped_topk_cpu(
  ...
  RECORD_FUNCTION(
      "sgl-kernel-custom::biased_grouped_topk_cpu", std::vector<c10::IValue>({hidden_states, gating_output, correction_bias}));
