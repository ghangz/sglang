...
def test_norm(batch_size, hidden_size, dtype, specify_out):
    ...
    if specify_out:
        ...
        sgl_kernel_custom.rmsnorm(x, w, out=y, enable_pdl=enable_pdl)
    else:
        y = sgl_kernel_custom.rmsnorm(x, w, enable_pdl=enable_pdl)
...
def test_fused_add_rmsnorm(batch_size, hidden_size, dtype):
    ...
    enable_pdl = is_arch_support_pdl()
    sgl_kernel_custom.fused_add_rmsnorm(
...
def test_gemma_norm(batch_size, hidden_size, dtype, specify_out):
    ...
    if specify_out:
        y = torch.empty_like(x)
        sgl_kernel_custom.gemma_rmsnorm(x, w, out=y, enable_pdl=enable_pdl)
    else:
        y = sgl_kernel_custom.gemma_rmsnorm(x, w, enable_pdl=enable_pdl)
...
def test_gemma_fused_add_rmsnorm(batch_size, hidden_size, dtype):
    ...
    residual_fused = residual.clone()
    enable_pdl = is_arch_support_pdl()
    sgl_kernel_custom.gemma_fused_add_rmsnorm(
        x_fused, residual_fused, weight, eps, enable_pdl=enable_pdl
    )
