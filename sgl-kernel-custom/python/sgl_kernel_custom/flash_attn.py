...
# try:
#     from sgl_kernel_custom import flash_ops
# except:
#     raise ImportError(
#         "Can not import FA3 in sgl_kernel_custom. Please check your installation."
#     )
...
def is_fa3_supported(device=None) -> bool:
    ...
    #  And for sgl-kernel-custom right now, we can build fa3 on sm80/sm86/sm89/sm90a.
...
def flash_attn_with_kvcache(
    ...
    out, softmax_lse, *rest = torch.ops.sgl_kernel_custom.fwd.default(
        q,
        k_cache,
        v_cache,
        k,
...
def flash_attn_varlen_func(
    ...
    if not is_fa3_supported():
        raise NotImplementedError(
            "flash_attn at sgl-kernel-custom is only supported on sm90 and above"
        )
