...
class MiniMaxM2RMSNormTP(nn.Module):
    ...
    def forward(
        ...
        if self.tp_world > 1:
            # All-reduce variance within attention TP group only (required for TP4+DP2)
            variance = get_attention_tp_group().all_reduce(variance) / self.tp_world
    ...
    def forward_qk(
        ...
        if q_norm.tp_world > 1:
            sum_sq = get_attention_tp_group().all_reduce(sum_sq)
...
class MiniMaxM2Attention(nn.Module):
    ...
    Uses attention TP (attn_tp_rank / attn_tp_size) for QKV/O and head split so that
    TP+DP (e.g. TP4+DP2) works: only attn_tp ranks share attention, not full tp group.
    """
    ...
    def __init__(
        ...
        if attn_tp_size > 1:
            self.q_norm = MiniMaxM2RMSNormTP(
                # self.total_num_heads * self.head_dim, eps=config.
                self.total_num_heads * self.head_dim,
