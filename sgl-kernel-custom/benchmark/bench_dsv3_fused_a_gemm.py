...
from sgl_kernel_custom import dsv3_fused_a_gemm

# CI environment uses simplified parameters
if IS_CI:
    num_tokens_vals = [1]  # Only test 1 value in CI
    line_vals = ["sgl-kernel-custom"]  # Only test sgl-kernel-custom implementation in CI
else:
    num_tokens_vals = [i + 1 for i in range(16)]  # Test 1-16 in full mode
    line_vals = ["torch", "sgl-kernel-custom"]
