...
std::tuple<at::Tensor, at::Tensor> chunk_gated_delta_rule_cpu(
    ...
    double eps = 1e-5) {
  RECORD_FUNCTION(
      "sgl-kernel-custom::chunk_gated_delta_rule_cpu", std::vector<c10::IValue>({query, key, value, g, beta, initial_state}));
...
at::Tensor fused_sigmoid_gating_delta_rule_update_cpu(
  ...
  RECORD_FUNCTION(
      "sgl-kernel-custom::fused_sigmoid_gating_delta_rule_update_cpu",
...
fused_gdn_gating_cpu(const at::Tensor& A_log, const at::Tensor& a, const at::Tensor& b, const at::Tensor& dt_bias) {
  RECORD_FUNCTION("sgl-kernel-custom::fused_gdn_gating_cpu", std::vector<c10::IValue>({A_log, a, b, dt_bias}));
  CHECK_DIM(1, A_log);
  CHECK_DIM(2, a);
  CHECK_DIM(2, b);
  CHECK_DIM(1, dt_bias);
