...
def create_random_cuda_tensor(shape, dtype, mean=0.4, std=0.1):
...
def test_fused_moe_once(M: int, N: int, K: int, Experts: int, topk: int, use_vllm: bool = False):
    ...
    w1 = torch.randint(-127, 127, (Experts, 2 * N, K), device = "cuda", dtype = torch.int8)
    w2 = torch.randint(-127, 127, (Experts, K, N), device = "cuda", dtype = torch.int8)
    w1_scale = torch.randn((Experts, 2 * N, 1), device = "cuda", dtype = torch.float32)
    w2_scale = torch.randn((Experts, K, 1), device = "cuda", dtype = torch.float32)
...
def main():
    # m_values: List[int] = [1, 7, 24, 28, 32, 1024, 6140, 7170, 8192]
    # m_values: List[int] = [1, 2, 4, 7, 8, 16, 24, 28, 32, 40, 56]
    m_values: List[int] = [1, 2, 4, 7, 8, 16, 24, 28, 32, 48, 56]
    n_values: int = 128
    k_values: int = 7168
    experts:  int = 256
    topk: int = 8
    ...
    # test_fused_moe_once(M = 1, N = n_values, K = 7168, Experts = experts, topk = topk)
    # test_fused_moe_once(M = 2, N = n_values, K = 7168, Experts = experts, topk = topk)
    # test_fused_moe_once(M = 4, N = n_values, K = 7168, Experts = experts, topk = topk)
    # test_fused_moe_once(M = 7, N = n_values, K = 7168, Experts = experts, topk = topk)
    # test_fused_moe_once(M = 8, N = n_values, K = 7168, Experts = experts, topk = topk)
    # test_fused_moe_once(M = 16, N = n_values, K = 7168, Experts = experts, topk = topk)
    # test_fused_moe_once(M = 24, N = n_values, K = 7168, Experts = experts, topk = topk)
    # test_fused_moe_once(M = 28, N = n_values, K = 7168, Experts = experts, topk = topk)
    # test_fused_moe_once(M = 32, N = n_values, K = 7168, Experts = experts, topk = topk)
    # test_fused_moe_once(M = 40, N = n_values, K = 7168, Experts = experts, topk = topk)
    # test_fused_moe_once(M = 48, N = n_values, K = 7168, Experts = experts, topk = topk)
    # test_fused_moe_once(M = 56, N = n_values, K = 7168, Experts = experts, topk = topk)
    # test_fused_moe_once(M = 1024, N = n_values, K = 7168, Experts = experts, topk = topk)
    # test_fused_moe_once(M = 6140, N = n_values, K = 7168, Experts = experts, topk = topk)
    # test_fused_moe_once(M = 7170, N = n_values, K = 7168, Experts = experts, topk = topk)
    # test_fused_moe_once(M = 8192, N = n_values, K = 7168, Experts = experts, topk = topk)
