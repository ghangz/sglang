...
from sgl_kernel_custom import gelu_and_mul, gelu_tanh_and_mul, silu_and_mul
...
try:
    from sgl_kernel_custom import gelu_quick
...
def calculate_diff(
    ...
    if kernel == "gelu_quick":
        ...
        test_out = getattr(sgl_kernel_custom, kernel)(x)
...
def benchmark(kernel, dtype, batch_size, seq_len, dim, provider):
    ...
    if kernel == "gelu_quick" and not GELU_QUICK_AVAILABLE:
        ...
        return (0, 0, 0)
    sglang_kernel = getattr(sgl_kernel_custom, kernel)
